...
router.get('/toDoList', function(request, response) {
    if(request.user == undefined) {
        ...
        var control = '';
        var list = db.get('toDoList').filter({user_id:request.user.shortid}).value();
        console.log("list-ft : ", list);
        if(1 <= list.length) {
...
router.post('/update_ToDoList_process', function(request, response) {
    ...
    for(var i = 0 ; i < listcount ; i++) {
        db.get('toDoList').remove({user_id:request.user.shortid, num:i}).write();
    }
    var post = request.body;
    var listCount = post.listCount;
    var list = new Array();
